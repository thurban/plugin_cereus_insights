...
/**
 * Return true when it is time to generate the weekly report.
 */
function cereus_insights_check_weekly_report(int $now, int $last_report_run): bool {
	...
	if (($now - $last_report_run) < 72000) return false; /* 20-hour minimum gap */
...
/**
 * Gather stats, call the LLM, store the report, send the email.
 */
function cereus_insights_generate_weekly_report(): void {
	...
	$anomaly_7d = db_fetch_row(
		"SELECT COUNT(*) AS total, COUNT(DISTINCT host_id) AS devices
	...
	$top_forecasts = db_fetch_assoc_prepared(
		"SELECT name_cache, datasource, forecast_days, ROUND(r_squared, 2) AS r_squared
	...
	/* ---- AI Alert Summaries (this week) ---- */
	$summary_stats = db_fetch_row(
		"SELECT COUNT(*)              AS total_summaries,
		        COALESCE(SUM(alert_count), 0)  AS total_alerts,
		        ROUND(AVG(alert_count), 1)     AS avg_alerts_per_batch
		 FROM plugin_cereus_insights_summaries
		 WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)"
	);

	$recent_summaries = db_fetch_assoc_prepared(
		"SELECT alert_count,
		        LEFT(summary, 250) AS summary_preview,
		        created_at
		 FROM plugin_cereus_insights_summaries
		 WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)
	...
	$payload = array(
		...
		'anomalous_devices_7d'        => (int)   ($anomaly_7d['devices']         ?? 0),
		...
		'alert_summaries_generated'   => (int)   ($summary_stats['total_summaries']      ?? 0),
		'alerts_summarized_7d'        => (int)   ($summary_stats['total_alerts']         ?? 0),
		'avg_alerts_per_summary'      => (float) ($summary_stats['avg_alerts_per_batch'] ?? 0),
		'recent_alert_summaries'      => $recent_summaries ?: array(),
	);
	...
	if (empty($api_key)) {
		cacti_log('CEREUS INSIGHTS: Weekly report skipped — no API key configured', false, 'SYSTEM');
		return;
	}
	...
	$system = 'You are an infrastructure monitoring assistant generating a weekly report for a network operations team. '
	        ...
	        . 'Write a concise weekly intelligence report with four sections: '
	        . '1. Capacity Concerns — top items approaching saturation with specific days remaining. '
	        . '2. Anomaly Highlights — devices with the most unusual activity this week. '
	        . '3. Alert Summary Review — key themes from the LLM-generated alert summaries this week; if none, note that. '
	        . '4. Action Items — 3-5 concrete recommendations ranked by urgency. '
	        . 'Keep each section to 2-4 sentences. Be specific: use device names, datasource names, and numbers from the data. '
	...
	$raw = cereus_insights_llm_dispatch($provider, $api_key, $model, $system,
		json_encode($payload, JSON_UNESCAPED_UNICODE), 800);
	...
	$subject = 'Cereus Insights: Weekly Report — '
	         . date('M j', strtotime($period_start)) . ' – ' . date('M j, Y', $now);
...
/**
 * Send the weekly report by email via Cacti's mailer.
 */
function cereus_insights_email_report(string $report_text, string $subject, array $payload): void {
	...
	/* Stats table shown at the bottom of the email */
	$c_crit = (int) ($payload['capacity_critical'] ?? 0);
	$stats_html =
		'<table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:16px;border-top:1px solid #eee;padding-top:12px;">'
		. '<tr>'
		.   '<td style="color:#666;padding:4px 8px;">Anomalies (7d)</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['anomalies_7d'] ?? 0) . '</td>'
		.   '<td style="color:#666;padding:4px 8px;">Devices affected</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['anomalous_devices_7d'] ?? 0) . '</td>'
		. '</tr>'
		. '<tr>'
		.   '<td style="color:#666;padding:4px 8px;">Capacity critical (&lt;7d)</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;color:' . ($c_crit > 0 ? '#e74c3c' : '#27ae60') . ';">' . $c_crit . '</td>'
		.   '<td style="color:#666;padding:4px 8px;">Capacity warning (&lt;30d)</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['capacity_warning'] ?? 0) . '</td>'
		. '</tr>'
		. '<tr>'
		.   '<td style="color:#666;padding:4px 8px;">Alert summaries (7d)</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['alert_summaries_generated'] ?? 0) . '</td>'
		.   '<td style="color:#666;padding:4px 8px;">Alerts summarized</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['alerts_summarized_7d'] ?? 0) . '</td>'
		. '</tr>'
		. '<tr>'
		.   '<td style="color:#666;padding:4px 8px;">Unconfigured datasources</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['unconfigured_datasources'] ?? 0) . '</td>'
		.   '<td style="color:#666;padding:4px 8px;">High-confidence suggestions</td>'
		.   '<td style="font-weight:bold;padding:4px 8px;">' . (int)($payload['high_confidence_suggestions'] ?? 0) . '</td>'
		. '</tr>'
		. '</table>';
	...
	$body_html =
		...
		.   '<strong>' . html_escape($subject) . '</strong>'
		. '</div>'
		. '<div style="border:1px solid #ddd;border-top:none;padding:20px;border-radius:0 0 4px 4px;">'
		.   '<p style="white-space:pre-wrap;line-height:1.8;margin:0 0 16px;">' . nl2br(html_escape($report_text)) . '</p>'
		.   $stats_html
		. '</div>'
		. '</div></body></html>';
